...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        temperatura = sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
        if(temperatura != null)
        {
            sensorManager.registerListener(MainActivity.this, temperatura,
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if(sensor.getType() == Sensor.TYPE_GYROSCOPE)
        {
            ...
            if(podeTrocar && (Math.abs(xAux-xAux2)>1 || Math.abs(yAux-yAux2)>1 || Math.abs(zAux-zAux2)>1)) {
                podeTrocar=false;
                x.setText("");
                y.setText("");
                z.setText("");
